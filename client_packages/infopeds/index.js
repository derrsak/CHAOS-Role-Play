...
let isInInfopedsShape = false;

mp.keys.bind(0x45, true, () => { /// E
    if (mp.game.ui.isPauseMenuActive()) return;
    if (mp.busy.includes()) return;
    if (!isInInfopedsShape) return;
...
mp.events.add({
    ...
    "infopeds.shape.enter": (state) => {
        isInInfopedsShape = state;

        if (state) mp.prompt.show(`Используйте <span>E</span> для того, чтобы поговорить`);
        else mp.prompt.hide();

        // Временно отключено, потому что новичков будут выталкивать
        // if (!state) mp.callCEFV(`modal.show = false`);
    }
});