...
mp.events.add('busdriver.rent.ans', (ans, data) => {
    ...
    switch (ans) {
        case 0:
            mp.notify.success('Вы арендовали транспорт');
            mp.events.call('busdriver.menu.show', data);
            break;
        case 1:
            mp.notify.error('Ошибка аренды');
            break;
        case 2:
            mp.notify.error('Недостаточно денег');
            break;
        case 3:
            mp.notify.error('Аренда невозможна');
            break;
        case 4:
            mp.notify.warning('Вы отказались от аренды');
            break;
    }
});
...
mp.events.add('busdriver.route.start.ans', (ans, data) => {
    ...
    switch (ans) {
        case 0:
            mp.notify.error('Это не рабочий транспорт');
            break;
        case 1:
            mp.notify.success('Маршрут построен');
            mp.notify.info('Чтобы окончить рабочий день, нажмите F6');
