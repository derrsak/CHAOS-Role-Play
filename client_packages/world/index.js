...
mp.events.add({
    ...
    "world.objects.getHash": () => {
        let hash = mp.utils.getFrontObjectHash(mp.players.local);
        if (hash) mp.utils.console(hash);
    },

    "utils.test.math": () => { // temp
        let test = Math.clamp(10, 1, 5); // val, min, max
        mp.utils.console(test);
    },
    ...
    "render": () => {
        ...
        if (mp.world.doorControl) {
            ...
            if (raycast) {
                ...
                text = (!isDoor) ? `~r~Объект не является дверью (${hash})` : `~g~Дверь найдена [E]`;
            }
