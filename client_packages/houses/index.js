...
mp.events.add('house.add.enter', () => {
    if (mp.players.local.vehicle) return mp.notify.error("Покиньте авто");
...
mp.events.add('house.add.carPlace', () => {
    if (!mp.players.local.vehicle) return mp.notify.error("Сядьте в авто");
...
mp.events.add('house.add.create', (interiorIndex, price) => {
    ...
    if (isNaN(interiorIndex)) return mp.notify.error("Ошибка в выборе интерьера");
    price = parseInt(price);
    if (isNaN(price)) return mp.notify.error("Введите стоимость корректно");
    if (enterMarker == null) return mp.notify.error("Создайте вход в дом");
    if (spawnMarker == null) return mp.notify.error("Создайте выход из дома");
    if (carPlaceMarker == null) return mp.notify.error("Создайте место спавна авто");
...
mp.events.add('house.add.interior.create', (garageId, className, numRooms, rent) => {
    ...
    if (isNaN(garageId)) return mp.notify.error("Ошибка в выборе гаража");
    if (className == "" || className == null) return mp.notify.error("Введите название класса жилья корректно");
    numRooms = parseInt(numRooms);
    if (isNaN(numRooms)) return mp.notify.error("Введите кол-во комнат корректно");
    rent = parseFloat(rent);
    if (isNaN(rent)) return mp.notify.error("Введите аренду корректно");
    if (exitMarker == null) return mp.notify.error("Создайте выход из интерьера");
    if (enterMarker == null) return mp.notify.error("Создайте вход в интерьер");
    if (holderMarker == null) return mp.notify.error("Создайте шкаф в интерьере");
...
mp.events.add('house.add.garage.close', () => {
    ...
    addGarageInfo.vehs.forEach(veh => mp.events.callRemote("house.add.carDrop", veh));
    addGarageInfo.GaragePlaces.forEach(place => {
        place.marker.destroy();
        delete place.marker;
        delete place.id;
    });
    addGarageInfo.GaragePlaces = [];
...
mp.events.add('house.add.carSpawn.ans', (vehId, id) => {
    const veh = mp.vehicles.atRemoteId(vehId);
    veh.idCar = id;
    addGarageInfo.vehs.push(id);
});
...
mp.events.add('house.add.garage.removePlace', () => {
    ...
    if (index == -1) return mp.notify.error("Авто не найдено в списке парковочных мест");
...
mp.events.add('house.add.garage.create', () => {
    if (addGarageInfo.carPlaces == 0) return mp.notify.error("Добавьте парковочные места");
    if (exitMarker == null) return mp.notify.error("Создайте выход из интерьера");
    if (enterMarker == null) return mp.notify.error("Создайте вход в интерьер");
    addGarageInfo.GaragePlaces.forEach(place => {
