...
let carSwapBootHoodList = ['nero'];

mp.utils = {
    /// Управление камерой
    cam: require('utils/camera.js'),

    /// Сообщение в консоль сервера
    console(text) {
    ...
    closeDoors() {
        ...
        // mp.game.object.doorControl(631614199, 461.8065, -994.4086, 25.06443, true, 0.0, 0.0, 0.0); // dublicate
        ...
        mp.game.object.doorControl(520341586, -14.86892, -1441.182, 31.19323, false, 0.0, 0.0, 0.01); // old: true | denis house
        ...
        // Bolingbroke
        mp.game.object.doorControl(741314661, 1844.998, 2597.482, 44.63626, true, 0.0, 0.0, 0.0); // prison
        ...
        /// Мэрия (BANK)
        mp.game.object.doorControl(110411286, 232.26393127441406, 214.656982421875, 106.28514099121094, false, 0.0, 0.0, 0.0); // old: true all
    ...
    requestIpls() {
        ...
        /// автосалон люкс
        mp.game.streaming.requestIpl("hei_showroom_open");
    },
    ...
    getHoodPosition(veh) {
        ...
        // if (hoodDist > 10) {
        //     if (mp.getInteractionClassesToIgnore().includes(veh.getClass())) {
        //         return null;
        //     }
        //     hoodDist = 2;
        // }
        ...
        let swapHood = carSwapBootHoodList.find(x => mp.game.joaat(x) == veh.model);
        let returnHoodPos = swapHood ? veh.getOffsetFromInWorldCoords(0, -hoodDist - 2, 0) : veh.getOffsetFromInWorldCoords(0, hoodDist + 2, 0);
        return returnHoodPos;
    },
    getBootPosition(veh) {
        if (!veh) return null;
        var vehPos = veh.position;
        ...
        let swapHood = carSwapBootHoodList.find(x => mp.game.joaat(x) == veh.model);
        let returnBootPos = swapHood ? veh.getOffsetFromInWorldCoords(0, bootDist + 2, 0) : veh.getOffsetFromInWorldCoords(0, -bootDist - 2, 0);
        return returnBootPos;
    },
    ...
    frontRaycast(player, draw = false) {
        ...
        return mp.raycasting.testPointToPoint(startPos, endPos, undefined, 16);
    },
    ...
    isEntityPlayingAnim(entity, dict, name, p = 3) {
...
// Переключение состояния курсора при нажатии клавиши ~ (0xC0)
mp.keys.bind(0xC0, true, () => {
    let visible = mp.gui.cursor.visible;
