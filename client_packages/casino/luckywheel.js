...
mp.events.add({
    "characterInit.done": () => {
        wheel = mp.objects.new(mp.game.joaat('vw_prop_vw_luckywheel_02a'), new mp.Vector3(1111.05, 229.81, -49.15), { dimension: casinoDimension });
    },
    "casino.luckywheel.roll": (playerId, index) => {
        wheel.setRotation(0.0, 0.0, 0.0, 1, true);

        const player = mp.players.atRemoteId(playerId);
        const pos = new mp.Vector3(1109.55, 228.88, -49.64);
        const dict = getAnimDict(player);
    ...
    "casino.luckywheel.enter": (state, price) => {
        isInLuckySheelShape = state;

        if (state) mp.prompt.show(`Нажмите <span>E</span> для того, чтобы крутить колесо за ${price} фишек`);
        else mp.prompt.hide();
    }
});
