...
mp.timer = {
                ...
                try {
                    if (timers[i].time <= Date.now()) {
                        ...
                        if (timers[i].interval != null) {
                            ...
                            if (logId) {
                                mp.utils.console(`Timer with id ${logId} removed before work`);
                            }
                            timers.splice(i, 1);
                            i--;
                        }
                        if (logId) {
                            mp.utils.console(`Timer with id ${logId} start`);
                        }
                        handler();
                        if (logId) {
                            mp.utils.console(`Timer with id ${logId} done`);
                        }
                    }
                } catch (error) {
                    timers.splice(i, 1);
                    i--;
                    mp.utils.console(`${JSON.stringify(error)}`);
                }
            }
        }, duration);
    ...
    add(handler, time, isInterval = false, isLog = false) {
        ...
        if (time === 0) {
            if (isLog) mp.utils.console(`Timer with timeout = 0 done`);
            handler();
            return;
        }
        let id = gId++;
        if (isLog) {
            mp.utils.console(`Add timer ${JSON.stringify({
                id: id,
                handler: handler,
                time: Date.now() + time,
    ...
    remove(timer) {
        ...
        if (index !== -1) {
            if (timers[index].isLog) {
                mp.utils.console(`Remove timer with id ${timers[index].id}`);
            }
