...
mp.factions = {
    ...
    boxHandler() {
        ...
        if (this.enableTakeBox) {
            if (this.hasBox()) return mp.notify.error(`Нельзя нести больше`);
        ...
        for (var name in positions) {
            ...
            switch (name) {
                case `blipColor`:
                    continue;
                case `holder`:
                    blipName = `Личные вещи`;
                    break;
                case `storage`:
                    blipName = `Инвентарь организации`;
                    break;
                case `warehouse`:
                    if (!positions[name]) continue; // если склада в организации нет технически
                    blipName = `Склад организации`;
                    break;
            }
