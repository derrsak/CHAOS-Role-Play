...
let isInClothingShopShape = false;
let pedModels = ['s_f_y_shop_low', 's_f_y_shop_mid', 's_f_m_shop_high'];
let npcScenario = 'WORLD_HUMAN_VALET';

mp.keys.bind(0x45, true, () => { /// E
    if (mp.game.ui.isPauseMenuActive()) return;
    if (mp.busy.includes()) return;
    if (!isInClothingShopShape) return;

    let player = mp.players.local;
    let vehicle = player.vehicle;
    if (vehicle) return;

    mp.events.callRemote('clothingShop.entering');
});

mp.events.add({
    'clothingShop.npc.create': (peds) => {
        peds.forEach(element => {
            element.model = pedModels[element.shopClass - 1];
            element.defaultScenario = npcScenario;
            mp.events.call('NPC.create', element);
        });
    },
    'clothingShop.shape.enter': () => {
        isInClothingShopShape = true;
        mp.prompt.show(`Используйте <span>E</span> для того, чтобы войти в примерочную`);
    },
    'clothingShop.shape.leave': () => {
        isInClothingShopShape = false;
    },
    ...
    'clothingShop.enter': (shopData) => {
        ...
        mp.busy.add('clothingShop', false);
