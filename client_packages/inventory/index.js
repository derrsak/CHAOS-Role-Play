mp.inventory = {
    ...
    initSearchItems(data) {
        ...
        if (!rec) return mp.notify.error(`Игрок #${data.playerId} не найден`);
    ...
    registerItemsAttachments(itemList) {

        itemList.forEach((item, index, array) => {
            const info = item.attachInfo;

            const pos = new mp.Vector3(info.pos[0], info.pos[1], info.pos[2]);
            const rot = new mp.Vector3(info.rot[0], info.rot[1], info.rot[2]);

            let attachAnim = null;
            if (info.anim) {
                const anim = this.animData[info.anim].split(" ");
                attachAnim = {
                    dict: anim[0],
                    name: anim[1],
                    speed: 8,
            ...
            mp.attachmentMngr.register(`invItem_${index + 1}`, item.model, info.bone, pos, rot, attachAnim);
        });
    ...
    //         setTimeout(() => { // ragemp 1.1, need timeout 100-200ms after mp.object.new
    //             try {
    //                 if (object) {
...
mp.events.add('inventory.hideItemFromHands', () => {
...
mp.events.add("inventory.registerItemsAttachments", (itemList) => mp.inventory.registerItemsAttachments(itemList)); // attach
