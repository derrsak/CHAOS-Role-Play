...
mp.getInteractionClassesToIgnore = () => {
...
function getClosestVehicle(pos, range = MAX_RANGE) {
    ...
    mp.vehicles.forEachInStreamRange((veh) => {
        ...
        if (distToVeh < range) {
            let vehArray = [{ pos: veh.position, dist: distToVeh }];
...
mp.events.add('characterInit.done', () => { /// G
...
mp.events.add('render', () => {
    ...
    try {
        ...
        if (!mp.players.local.vehicle) {
            mp.game.graphics.drawText("G", [position.x, position.y, position.z], {
                font: 4,
...
mp.events.add('interaction.money.accept', (value) => {
    ...
    if (!value) return mp.notify.error('Введите сумму');

    let sum = parseInt(value);
    if (isNaN(sum)) return mp.notify.error('Некорректная сумма');

    if (sum < 1) return mp.notify.error('Слишком малая сумма');

    if (sum > 500) return mp.notify.error('Слишком большая сумма');

    let targetId = currentInteractionEntity.remoteId;
    if (targetId == null) return mp.notify.error('Игрок не найден');
...
mp.events.add('interaction.money.ans', (ans) => {
    ...
    switch (ans) {
        case 0:
            mp.notify.error('Игрок не найден');
            break;
        case 1:
            mp.notify.error('Некорректная сумма');
            break;
        case 2:
            mp.notify.error('Недостаточно денег');
            break;
        case 3:
            mp.notify.error('Не удалось передать деньги');
            break;
        case 4:
            mp.notify.success('Вы передали деньги');
            break;
        case 5:
            mp.notify.error('Нельзя передать деньги себе');
            break;
    }
});
