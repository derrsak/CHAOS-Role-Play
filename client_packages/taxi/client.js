...
mp.events.add('taxi.client.app.search', () => {
    if (mp.players.local.vehicle) {
        mp.notify.error('Нельзя заказать такси из транспорта');
        mp.callCEFR('taxi.client.order.cancel', []);
        return;
    }
    let diff = Date.now() - lastCallTime;
    if (diff < minCallTime) {
        mp.notify.error(`Ожидайте ${parseInt((minCallTime - diff)/ 1000)} секунд`);
        mp.callCEFR('taxi.client.order.cancel', []);
        return;
    } 
...
mp.events.add('taxi.client.order.ans', (ans) => {
    switch (ans) {
        case 0:
            ...
            break;
        case 1:
            mp.notify.error('Вы не на улице');
            mp.callCEFR('taxi.client.order.cancel', []);
            break;
        case 2:
            mp.notify.error('У вас недостаточно наличности');
            mp.callCEFR('taxi.client.order.cancel', []);
            break;
        case 3:
            mp.notify.error('Вы уже сделали заказ');
            mp.callCEFR('taxi.client.order.cancel', []);
            break;
        case 4:
            mp.notify.success('Заказ отправлен');
            mp.notify.warning('Не покидайте зону, выделенную зеленым');
...
mp.events.add('taxi.client.waitshape.destroy', () => {
    ...
    if (waitShape) {
        waitShape.isTaxiClientShape = false;
        waitShape.destroy();
        waitShape = null;
    }
});
...
mp.events.add('taxi.client.waypoint.created', (position) => {
    mp.notify.info('Метка поставлена, подтвердите заказ в приложении');
...
mp.events.add('taxi.client.app.confirm.ans', (ans) => {
    switch (ans) {
        case 0:
            mp.notify.success('Данные переданы таксисту');
            break;
        case 1:
            mp.callCEFR('taxi.client.order.error', []);
            mp.notify.error('Недостаточно денег, выберите другое место');
            break;
    }
});
...
mp.events.add("characterInit.done", () => {
    mp.timer.addInterval(() => {
        try {
            ...
            mp.utils.console(JSON.stringify(err.message));
        }
    }, 100);
});

mp.events.add('taxi.client.order.canceled', () => {
    mp.notify.error('Заказ отменен водителем');
    mp.events.call('taxi.client.waitshape.destroy');
    mp.callCEFR('taxi.client.order.cancel', []);
});
...
mp.events.add('taxi.client.car.leave', () => {
    mp.notify.warning('Вы покинули такси, заказ отменен');
    mp.events.call('taxi.client.waitshape.destroy');
    mp.callCEFR('taxi.client.order.cancel', []);
});

mp.events.add('taxi.client.waitshape.leave', () => {
    mp.notify.warning('Вы покинули зону ожидания, заказ отменен');
    mp.events.call('taxi.client.waitshape.destroy');
    mp.callCEFR('taxi.client.order.cancel', []);
    mp.events.callRemote('taxi.client.order.cancel');
});
