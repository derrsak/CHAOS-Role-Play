...
mp.events.add('characterInit.start', () => {
...
let updateMarkers = function() {
    for (let i = 0; i < selectMarkers.length; i++) {
        ...
        selectMarkers[i] = mp.markers.new(2, new mp.Vector3(x, y, z + 1),
            ...
            direction: 0,
            rotation: new mp.Vector3(0, 180, 0),
            color: (i === currentCharacter) ? [255, 0, 0, 255] : [255, 20, 60, 120],
            visible: true,
            dimension: mp.players.local.dimension
        });
    }
...
/*        mp.console.logInfo(`${charInfo.hoursXP} + ${charInfo.mainLVL}`)*/
        mp.callCEFV(`characterInfo.addCharacter({
            ...
            mainLVL: "${charInfo.mainLVL}",
            hoursXP: "${charInfo.hoursXP}"
        });`)
    });
    mp.callCEFV(`characterInfo.show = true;`);
};
...
let choose = function() {
    ...
    if (isBinding) {
        ...
        mp.players.local.cloneToTarget(peds[currentCharacter].handle);
    }
};
...
let setCharClothes = function(indexPed) {
    ...
    mp.utils.clearAllView(peds[indexPed], charInfos[indexPed].hair); // раздеваем игрока полностью
    ...
    for (let i = 0; i < clothes.length; i++) {
        ...
        peds[indexPed].setComponentVariation(clothes[i][0], clothes[i][1], clothes[i][2], 0);
    }
    for (let i = 0; i < props.length; i++) {
        mp.players.local.setPropIndex(props[i][0], props[i][1], props[i][2], false);
        peds[indexPed].setPropIndex(props[i][0], props[i][1], props[i][2], false);
    }
};
...
let setCharTattoos = function(indexPed) {
    ...
    tattoos.forEach((tattoo) => {
        ...
        peds[indexPed].setDecoration(mp.game.joaat(tattoo.collection), mp.game.joaat(tattoo.hashName));
    });
};

let setCharCustom = function (indexPed) {
    if (charInfos.length <= indexPed) return;
    peds[indexPed].model = freemodeCharacters[charInfos[indexPed].gender];
    mp.players.local.model = freemodeCharacters[charInfos[indexPed].gender];

    mp.players.local.setHeadBlendData(charInfos[indexPed].mother, charInfos[indexPed].father, 0, 0, charInfos[indexPed].skin, 0, charInfos[indexPed].similarity, 1.0, 0.0, false);
    peds[indexPed].setHeadBlendData(charInfos[indexPed].mother, charInfos[indexPed].father, 0, 0, charInfos[indexPed].skin, 0, charInfos[indexPed].similarity, 1.0, 0.0, false);

    peds[indexPed].setComponentVariation(2, charInfos[indexPed].hair, 0, 2);
    peds[indexPed].setHairColor(charInfos[indexPed].hairColor, charInfos[indexPed].hairHighlightColor);
    peds[indexPed].setEyeColor(charInfos[indexPed].eyeColor);
    ...
    for (let i = 0; i < 10; i++) {
        mp.players.local.setHeadOverlay(i, charInfos[indexPed].Appearances[i].value, charInfos[indexPed].Appearances[i].opacity, colorForOverlayIdx(i, indexPed), 0);
        peds[indexPed].setHeadOverlay(i, charInfos[indexPed].Appearances[i].value, charInfos[indexPed].Appearances[i].opacity);
        peds[indexPed].setHeadOverlayColor(i, 1, colorForOverlayIdx(i, indexPed), 0);
    }
    for (let i = 0; i < 20; i++) {
        mp.players.local.setFaceFeature(i, charInfos[indexPed].Features[i].value);
        peds[indexPed].setFaceFeature(i, charInfos[indexPed].Features[i].value);
    }
};
