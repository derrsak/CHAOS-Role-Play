var inventory = new Vue({
    ...
    data: {
        ...
        itemsInfo: { // возможно это надо закомментить.
            1: {
                name: 'Очки',
        ...
        itemsMenu: {
            ...
            45: { // дрон
                'Использовать': {
                    handler(item) {
                        mp.trigger(`callRemote`, `drone.start`); // drone module
                    }
                }
            },
            54: { // веревка
        ...
        // Огнестрельные оружия (для зарядки патронами)
        ...
        // Еда
        eatList: [35, 126, 127, 128, 129, 132, 134, 142, 143, 144],
        // Напитки
        drinkList: [34, 130, 133, 145],
    ...
    methods: {
        ...
        getItemName(item) {
            ...
            if ([6, 7, 8, 9, 15, 133, 134].includes(item.itemId) && item.params.name) //сюда надо добавлять все предметы, которые теоретически могут менять названия
