var auth = new Vue({
    ...
    computed: {
        header() {
            var headers = ["Авторизация", "Регистрация", "Сброс пароля", "Подтверждение почты"];
            return headers[this.form];
        },
        info() {
            var infos = ["Чтобы продолжить игру на сервере, авторизуйтесь в свой аккаунт аккаунт", "Вам необходимо создать аккаунт, для дальнейшей игры на нашем проекте", "У Вас есть возможность восстановить свой аккаунт", "Подтвердите почту для дальнейшей игры на сервере"];
            return infos[this.form];
        },
