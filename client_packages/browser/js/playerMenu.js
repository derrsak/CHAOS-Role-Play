...
var playerMenu = new Vue({
    ...
    methods: {
        ...
        setChips(chips) {
            statistics["casinoChips"].value = chips;
        },
		setGexp(gexp) {
            statistics["gexp"].value = gexp;
        },
        setDonate(donate) {
            this.coins = donate;
        },
        ...
        setSettings(settings) {
            for (var key in settings) {
                ...
                if (key == 'chatTimestamp') mp.events.call("chat.time.set", !!settings[key]);
                else if (key == 'chatSize') mp.events.call("chat.size.set", settings[key] + 1);
