...
module.exports = {
    ...
    "carrier.load.products.buy": (player, data) => {
        ...
        if (veh.products && veh.products.bizOrder) return out(`Вы уже взяли заказ для бизнеса`);
        // if (veh.products && veh.products.count) return out(`Товар уже загружен`);
        var max = carrier.getProductsMax(player);
        if (data.count > max || (veh.products && data.count + veh.products.count > max)) return out(`Ваш навык не позволяет загрузить более ${max} ед.`);

        let type = ["grains", "soils"][data.product];
        let name = ["Зерно", "Удобрение"][data.product];
        if (veh.products && veh.products.type != type) return out(`Нельзя смешивать типы товаров, у вас загружено ${veh.products.name}`);

        var price = data.count * carrier.productPrice;
        if (player.character.cash < price) return out(`Необходимо $${price}`);

        if (veh.products && veh.products.count) data.count += veh.products.count;
    ...
    "carrier.load.products.sell": (player) => {
        ...
        if (!veh || !veh.db || veh.db.key != "job" || veh.db.owner != 4) return out(`Не в грузовике`);
        if (player.character.job != 4) return out(`Не на работе`);
        if (veh.products && veh.products.bizOrder) return out(`Отмените заказ для бизнеса`);
    ...
    "carrier.load.orders.take": (player, bizId) => {
        ...
        if (!veh || !veh.db || veh.db.key != "job" || veh.db.owner != 4) return out(`Не в грузовике`);
        if (player.character.job != 4) return out(`Не на работе`);
        if (veh.products && veh.products.count) return out(`У вас уже есть загруженные товары`);
