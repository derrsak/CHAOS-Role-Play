...
module.exports = {
    ...
    async spawnVehicle(veh, source) { /// source: 0 - спавн автомобиля из БД, 1 - респавн любого автомобиля, null - спавн админского авто и т. д.
        ...
        if (veh.color1 == 200) {
            vehicle.setColorRGB(veh.colorR1, veh.colorG1, veh.colorB1, veh.colorR2, veh.colorG2, veh.colorB2);
        } else {
        ...
        vehicle.colorR1 = veh.colorR1;
        vehicle.colorG1 = veh.colorG1;
        vehicle.colorB1 = veh.colorB1;
        vehicle.colorR2 = veh.colorR2;
        vehicle.colorG2 = veh.colorG2;
        vehicle.colorB2 = veh.colorB2
        vehicle.colorRN = veh.colorRN;
        vehicle.colorGN = veh.colorGN;
        vehicle.colorBN = veh.colorBN;
        vehicle.colorRxN = veh.colorRxN;
        vehicle.colorGxN = veh.colorGxN;
        vehicle.colorBxN = veh.colorBxN;
        vehicle.x = veh.x;
        vehicle.y = veh.y;
        vehicle.z = veh.z;
        vehicle.h = veh.h;
 ...
 //       vehicle.colorb2 = veh.colorb2;
        ...
        if (veh.key == 'job' || veh.key == 'newbie' || veh.key == 'farm') {
    ...
    async loadPrivateVehicles(player) {
        ...
        if (dbPrivate.length > 0) {
            ...
            if (hasHouse) {
                ...
                for (let i = 0; i < dbPrivate.length; i++)
                {
                    let veh = dbPrivate[i];
                ...
                // mp.events.call('parkings.notify', player, veh, 0); // зачем каждый раз уведомлять игрока при входе, что транспорт на парковке (при слете дома есть отдельное уведомление)
            }
