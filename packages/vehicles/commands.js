...
module.exports = {
    ...
    "/ex": {
        ...
        handler: (player, args, out) => {
            let vehicle = mp.vehicles.at(args[0]);
            if (!vehicle) return out.error('Транспорт не найден', player);
            vehicle.explode();
            out.info('Транспорт взорван', player);
        }
    },
    "/setveh": {
        access: 5,
        description: "Установить транспорт для новичков/фракции/работы/фермы",
        args: `["newbie"/"job"/"faction"/"farm"/"rent"] [id фракции/работы/фермы]`,
    ...
    "/repair": {
        ...
        handler: (player, args, out) => {
            ...
            if (!vehicle) return out.error('Транспорт не найден', player);
            vehicle.repair();
            out.info('Транспорт отремонтирован', player);
        }
    },
    "/dl": {
        access: 1,
        description: "Переключить режим отображения информации о транспортных средствах",
        args: ``,
        handler: (player, args) => {
            player.call("switchmode_infocars");
        }
    },
    "/delveh": {
        access: 1,
        description: "Удаление авто по айди",
        args: `[vehicleId]:n`,
        handler: (player, args, out) => {
            let vehicle = mp.vehicles.at(args[0]);
            if (!vehicle) return out.error('Транспорт не найден', player);
            vehicle.destroy();
            mp.game.vehicle.delete(vehicle); 
            out.info('Транспорт удалён', player);
        }
    },
}
