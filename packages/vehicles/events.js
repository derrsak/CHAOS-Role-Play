...
module.exports = {
    ...
    "playerEnterVehicle": (player, vehicle, seat) => {
        ...
        if (seat == 0 && vehicle.getVariable('isDrone')) return; // drone module
        ...
        if (!vehicle.engine && seat == 0 && !vehicle.isInGarage && vehicle.properties.vehType != 2) {
    ...
    "vehicles.engine.toggle": (player) => { /// Включение/выключение двигателя
        ...
        if (player.vehicle.properties.vehType == 2) return; // велосипед
    ...
    "vehicles.lock": (player, vehicleId) => {
        ...
        if (state) {
            ...
            vehicle.locked = true;
            player.call('notifications.push.warning', ['Вы закрыли транспорт', 'Успешно']);
        }
    },
    ...
    "vehicle.megafon.lspd1": (player) => {
        let idfrac = 2;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lspd2": (player) => {
        let idfrac = 2;
        let numbersound = 2;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lspd3": (player) => {
        let idfrac = 2;
        let numbersound = 3;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lssd1": (player) => {
        let idfrac = 3;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lssd2": (player) => {
        let idfrac = 3;
        let numbersound = 2;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lssd3": (player) => {
        let idfrac = 3;
        let numbersound = 3;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.lssd4": (player) => {
        let idfrac = 3;
        let numbersound = 4;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.gov1": (player) => {
        let idfrac = 1;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.gov2": (player) => {
        let idfrac = 1;
        let numbersound = 2;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.gov3": (player) => {
        let idfrac = 1;
        let numbersound = 3;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.gov4": (player) => {
        let idfrac = 1;
        let numbersound = 4;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.fib1": (player) => {
        let idfrac = 1;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.fib2": (player) => {
        let idfrac = 4;
        let numbersound = 2;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.fib3": (player) => {
        let idfrac = 4;
        let numbersound = 3;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.fib4": (player) => {
        let idfrac = 4;
        let numbersound = 4;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.ems1": (player) => {
        let idfrac = 5;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.army1": (player) => {
        let idfrac = 6;
        let numbersound = 1;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.army2": (player) => {
        let idfrac = 6;
        let numbersound = 2;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.army3": (player) => {
        let idfrac = 6;
        let numbersound = 3;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.army4": (player) => {
        let idfrac = 6;
        let numbersound = 4;
        mp.players.callInRange(player.position, 150, 'soundplay', [idfrac, numbersound]);
    },
    "vehicle.megafon.l": (player, vehicleId) => {
        if (!player.vehicle) return;
        let vehicle = mp.vehicles.at(vehicleId);
        if (!vehicle) return;
        if (player.character.factionId == 1) return player.call('vehicle.megafon.tol', [1]);
        if (player.character.factionId == 2) return player.call('vehicle.megafon.tol', [2]);
        if (player.character.factionId == 3) return player.call('vehicle.megafon.tol', [3]);
        if (player.character.factionId == 4) return player.call('vehicle.megafon.tol', [4]);
        if (player.character.factionId == 5) return player.call('vehicle.megafon.tol', [5]);
        if (player.character.factionId == 6) return player.call('vehicle.megafon.tol', [6]);
    },
    ...
    "vehicles.sell.offer.accept": (player, accept) => {
        ...
        if (accept) {
            ...
            const car = vehicles.getVehicleBySqlId(vehId);
            if (car && car.isInGarage) {
                target.call('notifications.push.error', ['Машина в гараже', 'Ошибка']);
                seller.call('notifications.push.error', ['Машина в гараже', 'Ошибка']);
                delete target.sellCarTargetOffer;
                delete seller.sellCarSenderOffer;
                return;
            }
