...
module.exports = {
        ...
        for (let i = 0; i < player.characters.length; i++) {
            ...
            charInfos.push({
                charInfo: {
                    ...
                    hoursXP: player.characters[i].hoursXP,
                    mainLVL: player.characters[i].mainLVL,
    ...
    create(player) {
        ...
        player.model = freemodeCharacters[1];
    ...
    async save(player, fullname, charData) {
        ...
        player.characterInfo.cash = 2000;
    ...
    async applyCharacter(player) {
        ...
        if (player.character) {
            ...
            const tattoo = await db.Models.CharacterTattoo.findAll({
                where: {
                    characterId: player.character.id
                }
            });

            tattoo.forEach(tattoo => {
                player.setDecoration(mp.joaat(tattoo.collection), mp.joaat(tattoo.hashName));
            });
    ...
    setStartClothes(player) {
        ...
        setTimeout(() => {
            player.call('inventory.hideItemFromHands', []);
        }, 500);
    },
    spawn(player) {
        ...
        switch (settings.spawn) {
            ...
            case 1: // дом
                ...
                player.spawn(pos);
                player.heading = house.info.Interior.rotation; // need test and 
