...
let bizUpdateCashBox = async function(id, money) {
    ...
    if (!bizesModules[biz.info.type]) return;
...
let destroyOrder = async function(id) {
    ...
    const isFactionBiz = bizesModules[biz.info.type].business.isFactionOwner;
    let player;
    if (!isFactionBiz) {
        player = mp.players.toArray().find(player => player != null && player.character != null && player.character.id === biz.info.characterId);
    }
...
module.exports = {
    ...
    blipsId: [361, 52, 93, 402, 73, 110, 71, 362, 75, 72, 0, 354],
    async addBiz(bizInfo) {
        
        if (bizInfo.type != 10) { // клубы на время нахер
            mp.blips.new(this.blipsId[bizInfo.type], new mp.Vector3(bizInfo.x, bizInfo.y, bizInfo.z), {
                name: bizInfo.name,
                shortRange: true,
                dimension: 0,
                color: 0 // todo: цвет в зависимости от типа
            });
        }
