...
module.exports = {
    ...
    "/tpf": {
        access: 1,
        description: "ТП относительно позиции",
        args: "[дальность]:n [высота]:n",
        handler: (player, args, out) => {
            let entity = (player.vehicle) ? player.vehicle : player;
            player.call('admin.tpf', [entity, args[0], args[1]]);
        }
    },
    "/veh": {
        access: 5,
        ...
        handler: async (player, args) => {
            if (vehicles != null) {
                ...
                setTimeout(() => player.call('vehicles.onGroundProperly.set', []), 100);
    ...
    "/pos": {
        access: 6,
        description: "Записать текущие координаты в файл",
        args: "",
        handler: (player, args) => {
            let comment = args.join(` `) || ``;
            let heading = player.vehicle ? player.vehicle.heading : player.heading;
            let content = `${player.position.x.toFixed(2)}, ${player.position.y.toFixed(2)}, ${player.position.z.toFixed(2)}, ${heading.toFixed(2)} | ${comment}\n`;

            fs.writeFile(`${__dirname}/pos.txt`, content, { flag: `a+` }, (err) => {
                if (err) return console.log(`[POS] Ошибка при сохранении координат. (${err})`);
            });
            player.call('chat.message.push', [`!{#ffffff} Координаты сохранены. ${comment}`]);
        }
    },
    "/timers": {
        access: 6,
    ...
    "/sethp": {
        ...
        handler: (player, args, out) => {
            ...
            rec.health = health;
            setTimeout(() => {
                out.info(`Игроку ${rec.name} установлено ${health} ед. здоровья`, player);
                notify.info(rec, `${player.name} установил вам ${health} ед. здоровья`);
            }, 200);
    ...
    "/tpmark": {
        access: 1,
        description: "Телепорт по метке на карте",
        args: "",
        handler: (player, args, out) => {
            player.call('admin.tpmark', [player, args[0], args[1]]);
        }
    },
}
