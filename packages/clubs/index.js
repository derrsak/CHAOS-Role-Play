...
module.exports = {
    ...
    createEnterMarker(club) {
        let pos = new mp.Vector3(club.db.enterX, club.db.enterY, club.db.enterZ);

        let enterMarker = mp.markers.new(1, new mp.Vector3(pos.x, pos.y, pos.z - 1), 0.5, {
            color: [0, 187, 255, 70]
        });
        ...
        colshape.onEnter = (player) => {
            ...
            player.position = new mp.Vector3(club.db.exitX, club.db.exitY + 2, club.db.exitZ);
    ...
    createExitMarker(club) {
        var pos = new mp.Vector3(club.db.exitX, club.db.exitY, club.db.exitZ);

        var exitMarker = mp.markers.new(1, new mp.Vector3(pos.x, pos.y, pos.z - 1), 0.5, {
            color: [0, 187, 255, 70],
            dimension: club.biz.info.factionId
        });

        var colshape = mp.colshapes.newSphere(pos.x, pos.y, pos.z, 1, exitMarker.dimension);
        colshape.onEnter = (player) => {
            if (player.vehicle) return;
            if (player.inClub) {
            ...
            player.dimension = 0;
            player.position = new mp.Vector3(club.db.enterX, club.db.enterY - 2, club.db.enterZ);
        };
        colshape.onExit = (player) => {
    ...
    openClub(player) {
        var club = this.clubs.find(x => x.biz.info.characterId == player.character.id);
        if (!club) return notifs.error(player, `Вы не владелец клуба`);
        const isOpen = !club.enterMarker.isOpen;
