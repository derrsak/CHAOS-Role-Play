...
module.exports = {
    ...
    "/pgivelic": {
        ...
        handler: (player, args, out) => {
            ...
            if (character.gunLicenseDate) return out.error(`${rec.name} уже имеет лицензию`, player);
            character.gunLicenseDate = new Date();
            character.save();

            out.info(`${player.name} выдал ${rec.name} лицензию на оружие `);
            notifs.info(rec, `${player.name} выдал вам лицензию на оружие`, `Админ`);
        }
    },
    "/ptakelic": {
        access: 6,
