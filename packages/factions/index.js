...
module.exports = {
    ...
    ammoVehModels: ["barracks", "barracks3", "moonbeam", "youga2", "insurgent2", "riot"],
    ...
    async loadFactionsFromDB() {
        ...
        console.log(`[FACTIONS] Организации загружены (${dbFactions.length} шт.)`);
    },
    initFactionMarkers() {
    ...
    createFactionMarker(faction) {
        ...
        let marker = mp.markers.new(1, pos, 0.5, {
            color: [255, 187, 0, 70],
            dimension: faction.d,
            visible: this.markersShow
        });
        marker.h = faction.h;
    ...
    getBlipsPos(faction) {
        ...
        if (positions.warehouse) positions.warehouse.d = this.getWarehouse(faction.id).dimension;
        return positions;
    },
    ...
    respawnVehicles(faction) {
        ...
        mp.vehicles.forEach(veh => {
            ...
            if (veh.db.owner != faction.id) return;
            if (vehicles.getOccupants(veh).length) return;
            if (veh.lastPlayerTime && start.getTime() - veh.lastPlayerTime < this.vehWaitSpawn) return;
