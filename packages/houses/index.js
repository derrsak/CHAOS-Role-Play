...
module.exports = {
    ...
    addHouse(houseInfo) {
        ...
        let carEnterPlace = mp.markers.new(36, new mp.Vector3(houseInfo.carX, houseInfo.carY, houseInfo.carZ + 0.6), 1, {
            rotation: new mp.Vector3(0, 0, 0),
            dimension: 0
        });
        let carEnterPlaceS = mp.colshapes.newSphere(houseInfo.carX, houseInfo.carY, houseInfo.carZ, 5.0, 0);

        carEnterPlaceS.onEnter = (player) => {
            if (!player.vehicle) return;
            if (houseInfo.characterId != player.character.id) return;
            player.call('vehicles.garage.3nter', [true]);
        };

        carEnterPlaceS.onExit = (player) => {
            if (!player.vehicle) return;
            if (houseInfo.characterId != player.character.id) return;
            player.call('vehicles.garage.3nter', [false]);
        };
