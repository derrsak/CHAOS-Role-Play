...
module.exports = {
    ...
    "/housedrop": {
        ...
        handler: (player, args) => {
            ...
            const house = houseService.getHouseById(parseInt(args[0]));
            if (!house) return;
            const houseId = house.info.id;

            houseService.dropHouse(house, false);
            player.call('notifications.push.success', [`Вы выселили владельца из дома #${houseId}`, "Успешно"]);
        }
    },
    "/houseremove": {
        access: 5,
