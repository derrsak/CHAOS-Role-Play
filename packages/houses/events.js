...
module.exports = {
    ...
    "house.buy": (player) => {
        ...
        money.removeCash(player, info.price, async function(result) {
            ...
            notifications.info(player, "Оплатите имущество в банке в течение 72 часов, иначе оно будет продано", "Внимание");
    ...
    "house.sell.check": (player, idOrNick) => {
        ...
        if (isNaN(id)) {
            ...
            if (!mp.players.exists(id)) return player.call("house.sell.check.ans", [null]);
    ...
    "house.add.carSpawn": async (player, i, garage) => {
        ...
        timer.add(() => {
            if (!mp.players.exists(player) || !mp.vehicles.exists(carPlaceVehicle[i])) return;

            player.putIntoVehicle(carPlaceVehicle[i], 0);
            if (garage) player.call("house.add.carSpawn.ans", [carPlaceVehicle[i].id, i]);
        }, 100);
    },
