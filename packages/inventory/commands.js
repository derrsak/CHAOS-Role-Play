...
module.exports = {
    ...
    "/invadditem": {
        access: 6,
        description: "Добавить предмет в инвентарь.",
        args: "[ид_игрока]:n [ид_предмета]:n [кол-во]:n",
        handler: (player, args, out) => {
            var rec = mp.players.at(args[0]);
            if (!rec || !rec.character) return out.error(`Игрок #${args[0]} не найден`, player);
            
            var item = inventory.inventoryItems[args[1] - 1];
            if (!item) return out.error(`Предмет #${args[1]} не найден`, player);

            inventory.addItem(rec, args[1], {count:parseInt(args[2])}, (e) => {
                if (e) return out.error(e, player);
            });
        }
    },
    "/invclearitems": {
