...
module.exports = {
    ...
    bodyList: {
        ...
        7: [10],
        8: [12],
        9: [21, 22, 48, 49, 50, 52, 70, 76, 91, 93, 96, 99, 100, 107, 136], // слот оружия
        10: [13],
        11: [8],
        12: [9],
    ...
    initInventoryConfig(player) {
        ...
        player.call(`inventory.registerItemsAttachments`, [this.inventoryItems]);
    ...
    cantAdd(player, itemId, params) {
        let item = this.getInventoryItem(itemId);
        let slot = this.findFreeSlot(player, itemId);
        if (!slot) return `Нет места для ${item.name}`;
        if (params.sex && params.sex != !player.character.gender) return `Предмет противоположного пола`;
        let nextWeight = this.getCommonWeight(player) + item.weight * (params.count || 1);
    ...
    getWeaponModels() {
        return this.bodyList[9].map(x => this.getInventoryItem(x) != undefined ? this.getInventoryItem(x).model : undefined);
    },
    ...
    updateView(player, item) {
        ...
        var clothesIndexes = { // "itemid": componentId
            "2": 7,
            "8": 4,
            "9": 6,
            "13": 5
        ...
        if (clothesIndexes[item.itemId] != null) {
            ...
            if (player.hasAttachment(`weapon_${item.itemId}`)) player.addAttachment(`weapon_${item.itemId}`, true);
    ...
    findFreeSlot(player, itemId) {
        ...
        var items = player.inventory.items;
        if (!this.getHandsItem(player)) return { // убрал, думаю нахер не надо в руки предмет давать, лучше сразу в инвентарь, выглядит не оч
            pocketIndex: null,
            index: 13,
            parentId: null
        };
    ...
    syncHandsItem(player, item) {
        ...
        if (item) { // вкл. синх. предмета/гана в руках
            ...
            if (params.weaponHash) {
                ...
                if (player.hands) {
                    player.addAttachment(`invItem_${player.hands.itemId}`, true);
                    delete player.hands;
                }
        
                if (player.vehicle) return;
                player.setVariable("hands", item.itemId);
                player.addAttachment(`invItem_${item.itemId}`);
                player.hands = { itemId: item.itemId };
            }
