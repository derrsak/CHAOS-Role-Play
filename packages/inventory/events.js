...
module.exports = {
        ...
        switch (item.itemId) {
            case 56: // канистра
                if (data.index == 0) { // заправить авто
                    ...
                    if (!products) return out(`На АЗС не хватает топлива`);
            ...
            case 139: // спички
                if (data.index == 0) { // костер
                    ...
                    if (!count) return notifs.error(player, `Количество спичек: 0 ед.`, header);

                    var firewoodCount = craft.getMaterialCount(player, craft.firewoodItemId);
                    if (firewoodCount < 3) return notifs.error(player, `Недостаточно дерева, нужно 3 шт.`, header);
    ...
    "playerSpawn": (player) => {
        if (player.character && player.inventory) inventory.updateAllView(player);
    },
    "playerDeath": (player) => {
    ...
    "inv.death.spawn": (player, pos, dimension) => {
        if (!player.character || !player.inventory) return;
