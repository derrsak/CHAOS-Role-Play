...
module.exports = {
	...
	getNamesList(player) {
		...
		player.familiar.list.forEach(familiar => {
			if (familiar.characterA == player.character.id) {
				if (familiar.characterTwo && familiar.characterTwo.name) {
					names.push(familiar.characterTwo.name);
				}
			} else {
				if (familiar.characterOne && familiar.characterOne.name) {
					names.push(familiar.characterOne.name);
				}
			}
		})
		return names
	...
	have(player, characterId) {
		var i = player.familiar.list.findIndex(
			x => x.characterA == characterId || x.characterB == characterId
		)
		return i != -1
	},
	add(player, rec) {
		...
		if (player.familiar.list.length > this.maxList)
			player.familiar.list.shift().destroy()
		if (rec.familiar.list.length > this.maxList)
			rec.familiar.list.shift().destroy()
	},
}
