...
module.exports = {
    ...
    "carservice.diagnostics.accept": (player, accept) => {
        ...
        let clearDiagnostics = (tTarget, tSender) => {
            delete tTarget.diagnosticsOffer;
            delete tSender.senderDiagnosticsOffer;
        }
        ...
        if (!accept) return clearDiagnostics(target, sender);
        ...
        if (productsAvailable < DEFAULT_DIAGNOSTICS_PRODUCTS) {
            ...
            clearDiagnostics(target, sender);
            return;
        }
        ...
        money.removeCash(target, price, (result) => {
            if (!result) {
            ...
            money.addMoney(sender, salary, (result) => {
                if (result) {
    ...
    "carservice.check.accept": (player, state) => {
        ...
        if (state) {
            ...
            money.removeCash(target, target.repairPrice, (result) => {
                if (result) {
                    ...
                    money.addMoney(mechanic, parseInt(salary * (1 + bonus)), (result) => {
                        if (result) {
