...
module.exports = {
    ...
    "news.storage.clothes.take": (player, index) => {
        if (!player.insideFactionWarehouse) return notifs.error(player, `Вы далеко`, `Склад Lifeinvader`);
        if (!factions.isNewsFaction(player.character.factionId)) return notifs.error(player, `Вы не редактор`, `Склад Lifeinvader`);
