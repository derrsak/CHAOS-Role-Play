...
let utils = {
    ...
    sendMail(to, subject, message) {
        ...
        var transporter = nodemailer.createTransport({
            // service: mp.config.emailService,
            host: mp.config.smtpHost,
            port: mp.config.smtpPort,
            secure: mp.config.smtpPort == 587 ? false : true,
            auth: {
                user: mp.config.emailLogin,
                pass: mp.config.emailPass
            },
            tls: {
                ciphers:'SSLv3'
            }
        });
        message += "<br /><br / > С уважением, команда Chaos Project.";
        const mailOptions = {
            from: `Chaos Project <${mp.config.emailLogin}>`,
            to: to,
            subject: subject,
            html: message
        };
