...
module.exports = {
    ...
    add(player, listenerId) {
        if (listenerId === player.id) {
            ...
            return notify.error(mp.players.at(listenerId), `Нельзя установить прослушку на свой номер`);
        }
        ...
        notify.info(mp.players.at(listenerId), `Прослушка подключена`);
    },
    remove(player, currentListenerId) {
        ...
        if (listenerIdIndex !== -1) {
            ...
            notify.info(mp.players.at(currentListenerId), `Прослушка отключена`);
        }
    ...
    callStart(target, subTarget) {
        target.wiretapping.listenersId.forEach(listenerId => {
            ...
            notify.info(mp.players.at(listenerId), `Человеку с номером телефона ${target.phone.number} поступил звонок`);
        });
    },
    callEnd(target, subTarget) {
        target.wiretapping.listenersId.forEach(listenerId => {
            ...
            notify.info(mp.players.at(listenerId), `У человека с номером телефона ${target.phone.number} окончен звонок`);
        });
    }
};