...
module.exports = {
    ...
    "wiretapping.add": (player, phoneNumber) => {
        if (!phone.isExists(phoneNumber)) {
            ...
            return notify.error(player, `Человека с таким номером не существует`);
        }
        let target = phone.findPlayerByPhoneNumber(phoneNumber);
        if (target == null) {
            player.call("wiretapping.result", [false]);
            return notify.error(player, `Человек с таким номером вне зоны действия сети`);
        }
    ...
    "playerQuit": player => {
        if (player.wiretapping != null) {
            player.wiretapping.listenersId.forEach(listenerId => {
                ...
                notify.error(player, `Человек с номером ${player.phone.number} пропал из сети`);
