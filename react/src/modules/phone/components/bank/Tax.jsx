...
const Tax = ({house, biz, bankApp, addAppAction}) => {
    ...
    const pay = () => {
        if (biz) {
            ...
            addAppAction(<Confirm info={{ type: 'biz', sum: selectedDays*biz.rent, name: name, id: biz.id, days: selectedDays }} />);
        }

        if (house) {
            const name = `Дом ${house.name}`;

            addAppAction(<Confirm info={{ type: 'house', sum: selectedDays*house.rent, name: name, id: house.name, days: selectedDays }} />);
        }
    };

    const validate = (info, money) => {
        const days = parseInt(selectedDays);
        if (selectedDays === 0 || days < 1) return setError('Количество дней не выбрано');
        setError('');

        if (days > (30 - info.days)) return setError(`Вы можете оплатить максимум ${(30 - info.days)} дней`);
        setError('');

        if (parseInt(money) < (days * info.rent)) return setError('Недостаточно денежных средств');
        setError('');
